...
const PARAM_NAME = "/appconfig/mercury";
...
async function getDbConfig(): Promise<DbConfig> {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("readonlydb =")) {
      ...
      password = decodePassword(userPass.substring(colonIdx + 1));
    }
  }
...
if (!sql) {
  console.error("Usage: ./query-mercury.ts <SQL query>");
  console.error('Example: ./query-mercury.ts "SELECT * FROM shops LIMIT 5"');
  process.exit(1);
}
